  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      if (errorParam) {
        ...
        setTimeout(
          () => navigate(isLinking ? "/github-integration" : "/login"),
          3000,
        );
        return;
      }
      ...
      try {
        if (isLinking) {
          ...
          const content = (data?.content ?? data?.Content ?? data) as
            | Record<string, unknown>
            | undefined;
          const inner = (content?.content ?? content?.Content ?? content) as
            | Record<string, unknown>
            | undefined;
          const token =
          ...
          if (
            ...
            try {
              ...
              console.error(
                "getMe after GitHub login failed",
                err.response?.status,
                err.response?.data,
              );
              setError(
        ...
        const error = err as {
          response?: { data?: { message?: string; content?: any } };
          message?: string;
        };
        const errorMessage =
