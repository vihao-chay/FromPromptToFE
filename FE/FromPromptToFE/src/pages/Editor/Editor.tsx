...
const Editor: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const runningTasks = TASK_IDS.map((t, i) => ({ ...t, status: 'Running' as TaskStatus, progress: i === 0 ? 100 : 50 }));
      setTasks((prev) => prev.map((t, i) => (i <= 1 ? { ...t, status: 'Running' as TaskStatus, progress: i === 0 ? 100 : 50 } : t)));
      setChatTurns((prev) => prev.map((turn) => (turn.id === assistantTurnId && turn.role === 'assistant' ? { ...turn, tasks: runningTasks } : turn)));
      ...
      const finalTasks = TASK_IDS.map((t, i) => ({ id: t.id, label: t.label, status: (isError ? (i === 0 ? 'Failed' : 'Pending') : 'Success') as TaskStatus, progress: isError ? (i === 0 ? 0 : 0) : 100 }));
  ...
  const handleCopy = () => {
    ...
    const fallback = activeCodeTab === 'tsx' ? '// Run to generate.' : '<!-- Run to generate. -->';
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className={`flex flex-col ${showOutputPanel ? 'w-[48%]' : 'flex-1'} min-w-0 transition-all duration-200`}>
          <div className="flex-1 flex flex-col min-h-0 max-w-4xl mx-auto w-full">
            {/* Chat log: prompt + logs ngay trên */}
            ...
            {/* Ô nhập kiểu ChatGPT: + bên trong trái, placeholder giữa, nút gửi bên trong phải */}
            <div className="p-4 pt-0">
              ...
              <div className="flex items-end gap-0 rounded-2xl border border-slate-200 dark:border-[#282e39] bg-white dark:bg-[#1c1f27] shadow-sm overflow-hidden focus-within:ring-2 focus-within:ring-primary/30">
                ...
                <textarea
                  ...
                  placeholder="Hỏi bất kỳ điều gì... Ví dụ: Tạo dashboard với sidebar và thống kê."
                  className="flex-1 min-h-[52px] max-h-[200px] resize-none py-3 px-2 text-sm text-slate-900 dark:text-white bg-transparent border-0 focus:ring-0 placeholder:text-slate-400 dark:placeholder:text-[#4d576e] custom-scrollbar"
                  onKeyDown={(e) => {
                ...
                <button
                  ...
                  className="flex-shrink-0 p-3 text-slate-500 dark:text-slate-400 hover:text-primary hover:bg-primary/5 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:text-slate-500 disabled:hover:bg-transparent transition-colors"
                  title="Gửi"
                >
                  {isSaving ? (
        ...
        {/* Right: Chỉ hiện khi đã Run — lúc chạy chỉ status, xong mới có 3 tab */}
        {showOutputPanel && (
          <div className="flex-1 flex flex-col min-w-0 border-l border-slate-200 dark:border-[#282e39] bg-white dark:bg-[#1c1f27]">
            {isSaving ? (
              <div className="flex-1 overflow-auto p-4">
                <p className="text-sm font-medium text-slate-700 dark:text-slate-200 mb-4">Tiến trình</p>
                <ul className="space-y-3">
                  {tasks.map((task) => (
                    <li key={task.id} className="rounded-lg border border-slate-200 dark:border-[#282e39] bg-slate-50 dark:bg-[#161921] p-3">
                      <div className="flex justify-between items-center mb-1.5">
                        <span className="text-sm text-slate-900 dark:text-white">{task.label}</span>
                        <span className={`px-2 py-0.5 rounded text-xs font-medium border ${statusClass(task.status)}`}>{task.status}</span>
                      </div>
                      <div className="h-1.5 rounded-full bg-slate-200 dark:bg-[#282e39] overflow-hidden">
                ...
                <div className="flex border-b border-slate-200 dark:border-[#282e39]">
                  {(['code', 'preview', 'tasks'] as const).map((tab) => (
                    <button
                      ...
                      className={`flex-1 min-h-[40px] flex items-center justify-center gap-1.5 px-3 text-xs font-medium border-b-2 transition-colors ${
                        outputTab === tab ? 'border-primary text-primary' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-white'
                      }`}
                    >
                      {tab === 'code' && <span className="material-symbols-outlined text-base">code</span>}
                      {tab === 'preview' && <span className="material-symbols-outlined text-base">desktop_windows</span>}
                      {tab === 'tasks' && <span className="material-symbols-outlined text-base">list</span>}
                      {tab === 'code' && 'Code'}
                      {tab === 'preview' && 'Preview'}
                      {tab === 'tasks' && 'Status'}
                    </button>
                ...
                <div className="flex-1 overflow-hidden flex flex-col min-h-0">
                  {outputTab === 'code' && (
                    <div className="flex flex-col h-full">
                      <div className="flex items-center justify-between px-3 py-2 border-b border-slate-200 dark:border-[#282e39] bg-slate-50 dark:bg-[#161921]">
                        <div className="flex gap-1">
                          <button
                            type="button"
                            onClick={() => setActiveCodeTab('tsx')}
                            className={`px-2.5 py-1 rounded text-xs font-mono ${activeCodeTab === 'tsx' ? 'bg-primary/20 text-primary' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-[#282e39]'}`}
                          >
                            index.tsx
                          </button>
                          <button
                            ...
                            className={`px-2.5 py-1 rounded text-xs font-mono ${activeCodeTab === 'html' ? 'bg-primary/20 text-primary' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-[#282e39]'}`}
                          >
                            index.html
                          </button>
                        </div>
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            onClick={handleCopy}
                            className="flex items-center gap-1 px-2.5 py-1 rounded border border-slate-200 dark:border-[#3b4354] text-xs font-medium hover:bg-slate-100 dark:hover:bg-[#282e39]"
                          >
                            <span className="material-symbols-outlined text-sm">content_copy</span>
                            {copied ? 'Copied!' : 'Copy'}
                          </button>
                          <Link to="/github-integration" className="flex items-center gap-1 px-2.5 py-1 rounded bg-primary text-white text-xs font-semibold hover:bg-primary/90">
                            <span className="material-symbols-outlined text-sm">upload</span>
                            GitHub
                          </Link>
                        </div>
                      </div>
                      <pre className="flex-1 overflow-auto p-3 font-mono text-xs text-slate-700 dark:text-[#9da6b9] bg-[#0d1117] whitespace-pre">
                        {activeCodeTab === 'tsx' ? generatedTsx : generatedHtml}
                      </pre>
                    </div>
                  )}
                  {outputTab === 'preview' && (
                    <div className="flex flex-col h-full p-3">
                      <div className="flex justify-end gap-1 mb-2">
                        {[
                          ...
                          <button
                            ...
                            className={`p-2 rounded ${device === id ? 'bg-primary/20 text-primary' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-[#282e39]'}`}
                          >
                      ...
                      <div
                        className={`flex-1 rounded-lg border border-slate-200 dark:border-[#282e39] bg-white dark:bg-slate-900 overflow-hidden ${
                          device === 'desktop' ? 'w-full' : device === 'tablet' ? 'max-w-[768px] mx-auto w-full' : 'max-w-[375px] mx-auto w-full'
                        }`}
                      >
                        <iframe
                          title="Preview"
                          srcDoc={generatedHtml || "<html><body style='margin:0;padding:24px'><p>Preview</p></body></html>"}
              ...
              <div className="flex-1 flex items-center justify-center text-sm text-slate-500 dark:text-slate-400">
                Đã chạy nhưng chưa có kết quả. Thử lại với prompt khác.
              </div>
            )}
          </div>
        )}
...
function SectionCard({
  ...
  return (
    <div className="rounded-lg border border-slate-200 dark:border-[#282e39] overflow-hidden bg-slate-50/50 dark:bg-[#161921]">
      <div className="flex items-center justify-between px-2.5 py-1 border-b border-slate-200 dark:border-[#282e39]">
        <span className="text-[10px] text-slate-500 uppercase">{subtitle}</span>
        <span className="text-xs font-semibold text-slate-800 dark:text-white">{title}</span>
      </div>
      <textarea
        value={value}
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
        className="w-full min-h-[60px] resize-y p-2 text-xs font-mono text-slate-900 dark:text-white bg-transparent border-0 focus:ring-0 placeholder:text-slate-400"
      />
    </div>
  );
}
