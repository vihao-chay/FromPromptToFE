...
export default function AdminUsersScreen({ navigation }) {
  ...
  const fetchUsers = useCallback(async (currentPage, currentSearch) => {
    ...
    try {
      ...
      const list =
        content?.totalItems ?? content?.TotalItems ?? content?.totalitems ?? [];
  ...
  return (
    <View style={styles.container}>
      <View
        style={[styles.header, { paddingTop: Math.max(14, insets.top + 6) }]}
      >
        <TouchableOpacity
            ...
            {users.map((user) => (
              ...
              >
                <View
                  style={[
                    styles.row,
                    selectedIds.includes(user.id) && styles.rowSelected,
                  ]}
                >
                  <TouchableOpacity
                  ...
                  >
                    <View
                      style={[
                        styles.checkbox,
                        selectedIds.includes(user.id) && styles.checkboxChecked,
                      ]}
                    >
                  ...
                  <Text style={styles.cellProvider} numberOfLines={1}>
                    {user.provider}
                  </Text>
                  ...
                  <View style={styles.cellRole}>
                    <Text
                      style={[styles.badge, user.isAdmin && styles.badgeAdmin]}
                    >
                  ...
                  <View style={styles.cellActions}>
                    ...
                    <TouchableOpacity onPress={() => setEditingUser(user)}>
                      <Text style={[styles.actionLink, { color: "#3b82f6" }]}>
                        Edit
                      </Text>
                    </TouchableOpacity>
                    <TouchableOpacity
      ...
      <EditUserModal
        ...
        onUserUpdated={(updated) => {
          setUsers((prev) =>
            prev.map((u) =>
              u.id === editingUser?.id ? { ...u, ...normUser(updated) } : u,
            ),
          );
          setEditingUser(null);
        }}
      ...
      <ConfirmModal
        ...
        message={
          confirmDeleteUser
            ? `Delete user ${confirmDeleteUser.email}? This action cannot be undone.`
            : ""
        }
